...
| **TC-PAIR-06** | P1 | **AP mode.** Pre: tub reset until its indicator blinks **slowly**. Steps: switch to *Wi-Fi hotspot (AP)*, enter the **home router** Wi-Fi (not the tub's own hotspot), follow the on-screen steps to join the `SmartLife…` hotspot, return, Start searching. Expect: pairing completes and the phone returns to its normal Wi-Fi. |
| **TC-PAIR-07** | P2 | **AP does not pre-fill.** Steps: switch EZ → AP. Expect: the Wi-Fi field is **cleared**, and a warning says to enter the home network, not the device hotspot. |
