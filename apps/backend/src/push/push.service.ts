...
export type PushPayload = {
  title: string;
  body: string;
  image?: string; // URL ảnh hiển thị trong notification (FCM notification.imageUrl)
  data?: Record<string, string>; // deeplink / tap-routing ở app (vd { screen, devId, url })
};
...
export type BatchSendResult = {
  ...
  success: number;
  failed: number;
  deliveredUids: string[]; // uid có ≥1 thiết bị NHẬN được (để log lịch sử đúng người thật sự nhận, không ghi noti trượt)
};
...
export class PushService {
  ...
  async sendToUid(tuyaUid: string, payload: PushPayload): Promise<SendResult> {
    ...
    const res = await this.messaging.sendEachForMulticast({
      ...
      notification: {
        title: payload.title,
        body: payload.body,
        imageUrl: payload.image, // ảnh lớn trong notification (Android/iOS)
      },
      data: payload.data,
    });
  ...
  async sendToUids(
    ...
    for (const uid of uids) {
      ...
      if (res.sent > 0) {
        success += 1;
        deliveredUids.push(uid);
      } else {
        failed += 1;
      }
    }
    return { total: uids.length, success, failed, deliveredUids };
  }
