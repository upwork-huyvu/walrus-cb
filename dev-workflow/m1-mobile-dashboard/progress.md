...
✅ **B7–B9 XONG code** (audit fix). Còn lại: (1) khi có mạng `yarn install` → chạy `jest` (5 file: deviceSchema,
deviceMachine, tuyaError, debounce, dp) + `tsc`; (2) **AC6** + **HW1–HW8** trên thiết bị thật; (3) **H-2 DP schema thật**
(chờ client). Sau đó mới đủ điều kiện FINISH. Nit L-1/L-2/L-3 ở backlog (không làm trong feature).
...
- [◑] AC8 — (audit H-1) lỗi SDK log+map mã → thông điệp phân biệt; không empty-catch nuốt code · **code xong** (verify jest/tsc)
- [◑] AC9 — (audit M-1/M-2) debounce publish target + timeout đọc snapshot · **code xong** (verify jest/tsc)
...
- ✅ **(ĐÃ GỠ 2026-06-30)** Module `apps/mobile/src/lib/format` từng thiếu → đã **khôi phục** từ `replit_generate/App.js`
  (nguồn tham chiếu canonical): `LOGO_URI`/`LOGO_URI_LIGHT` (Dropbox) + `formatTime`. App + 4 screen compile lại được.
